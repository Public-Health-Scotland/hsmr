...
get_t3_data = function(date_str, sheet_name = "Raw Data"){
  ...
  data = 
    ...
    select(location_name, quarter_short, label, 
...
t3_old = get_t3_data(previous_pub, sheet_name = "data")
t3_new = get_t3_data(pub_day)
...
overlap_quarters = 
  semi_join(t3_new, t3_old, by = "quarter_short") %>%
...
t3_old = filter(t3_old, quarter_short %in% overlap_quarters$quarter_short)
t3_new = filter(t3_new, quarter_short %in% overlap_quarters$quarter_short)
...
There may be a higher number of changes in the first quarter, as some of the deaths in the first quarter of the previous release will have rolled over into the first quarter in this release; patients may have an admission in quarter A, another in quarter B, then die within 30 days of their quarter A admission. The death would have been assigned to quarter A in the previous release and quarter B in this release if quarter A is no longer included.

There may be a higher number of changes in the last quarter since that is the most recent in the previous publication and therefore the most likely to have seen changes to the SMR data.
...
join_cols = c("location_name", "quarter_short", "label")
