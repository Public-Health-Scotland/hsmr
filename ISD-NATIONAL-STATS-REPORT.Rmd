...
trend_data <- read_csv(here("data/output",
                            paste0(pub_date(end_date = end_date,
                                            pub = "current"),
                                   "_trends-data.csv"))) %>%
  filter(!is.na(label)) %>% 
  arrange(hb2014, label, quarter) 
...
trend_palette <- scale_color_manual(values = c("#26456E", "#43719F", "#5485B2", "#6A9BC3", "#79AACF", "#92C0DF", "#B9DDF1"))

funnel_plot <- ggplot(smr_data %>% filter(location_type == "hospital"), aes()) + 
  scale_color_manual(values = c("#006ba4", "#c85302", "#ffbc79", "#006ba4", "#ffbc79", "#c85302"), guide = FALSE) +
  ...
  geom_point(aes(x = pred, y = smr, colour = factor(flag), show.legend = FALSE)) + 
...
chart_3 <- ggplot(trend_data %>% filter(label == "Elective" | label == "Non-Elective"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
...
chart_4 <- ggplot(trend_data %>% filter(label == "Community" | label == "Dental" | label == "Medical" | label == "Other" | label == "Survery" | label == "Emergency" | label == "Women & Newborn"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter)), y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
  coord_cartesian(ylim = c(0, 7)) +
  labs(x = "Quarter", y = "Crude Rate (%)") +
...
chart_5 <- ggplot(trend_data %>% filter(label == "0-19 years" | label == "20-39 years" | label == "40-59 years" | label == "60-79 years" | label == "80+ years"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
...
chart_7 <- ggplot(trend_data %>% filter(label == "1 - Most Deprived" | label == "2" | label == "3" | label == "4" | label == "5 - Least Deprived"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
...
chart_8 <- ggplot(trend_data %>% filter(label == "Discharge" & hb2014 == "Scotland"), 
                  aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                      , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
...
chart_9 <- ggplot(trend_data %>% filter(label == "Population" & hb2014 == "Scotland"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
...
ggsave("Chart_9.png", plot = chart_9, width = 17.49, height = 9.03, units = "cm", 
       device = "png", dpi = 600)
...
![Trend chart showing crude mortality by quarter for latest three years.](Chart_2.png)
...
<Div custom-style = "Heading 4">Chart 4: Crude mortality rates (%) for deaths within 30-days of admission by age group; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in Table 3
![Test Alt Text](Chart_4.png)
...
<Div custom-style = "Heading 4">Chart 5: Crude mortality rates (%) for deaths within 30-days of admission by gender; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in Table 3
...
<Div custom-style = "Heading 4">Chart 6: Crude mortality rates (%) for deaths within 30-days of admission by deprivation; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in Table 3
