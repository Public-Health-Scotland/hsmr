...
latest_deaths_admission <- trend_data %>% 
  filter(hb2014 == "Scotland" & label == "All Admissions" & quarter == 20)
  
latest_deaths_discharge <- trend_data %>% 
  filter(hb2014 == "Scotland" & label == "Discharge" & quarter == 20)

# Calculates completeness date (has been put into a function on another branch
# so will be obsolete/should be deleted once merged)
...
chart_7 <- ggplot(trend_data %>% filter(label == "1 - Most Deprived" 
                                        | label == "2" | label == "3" 
                                        | label == "4" 
...
chart_9 <- ggplot(trend_data %>% filter(label == "Population" 
                                        & hb2014 == "Scotland"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
