...
funnel_plot <- ggplot(smr_data %>% filter(location_type == "hospital"), aes()) + 
  ...
  geom_point(aes(x = pred, y = smr, colour = factor(flag), 
                 show.legend = FALSE)) + 
  geom_text(aes(x = pred, y = smr, 
                label = ifelse(flag_below, as.character(location_name), '')), 
            hjust = 0.5, vjust = 1.5) +
  geom_text(aes(x = pred, y = smr, 
                label = ifelse(flag_above, as.character(location_name), '')), 
            hjust = 0.5, vjust = -1.5) +
...
The main changes made to the HSMR methodology at this time were to:  
...
Chart 1 shows the funnel plot for the latest 12 month period. This shows that there are no hospitals above the upper control limit which is three standard deviations above the Scottish average. 
