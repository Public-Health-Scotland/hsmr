...
chart_2 <- ggplot(trend_data %>% filter(label == "All Admissions" 
                  ...
                  aes(x = forcats::fct_reorder(quarter_short,
                      ...
                      group = 1)) +
  trend_palette +
  geom_line(aes(colour = "#26456E")) +
  coord_cartesian(ylim = c(0, 4)) +
  labs(x = "Quarter", y = "Crude Rate (%)") +
  theme(panel.background = element_blank(),
    panel.grid.major.x = element_line(size = .1, color = "#C0C0C0"), 
    panel.grid.major.y = element_blank(),
    axis.title.x = element_text(size = 12, face = "bold"),
...
chart_3 <- ggplot(trend_data %>% filter(label == "Elective" | 
                                          label == "Non-Elective"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
  ...
  theme(panel.background = element_blank(),
    ...
    axis.title.x = element_text(size = 12, face = "bold"),
    axis.title.y = element_text(size = 12, face = "bold"),
    axis.text = element_text(size = 12, angle= 90, hjust = 0, vjust = 0.5))

chart_4 <- ggplot(trend_data %>% filter(label == "Community" | label == "Dental"
...
chart_5 <- ggplot(trend_data %>% filter(label == "0-19 years" 
                                        | label == "20-39 years" 
                                        | label == "40-59 years" 
                                        | label == "60-79 years" 
...
chart_6 <- ggplot(trend_data %>% filter(label == "Male" | label == "Female"),
                  aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                      , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
...
chart_7 <- ggplot(trend_data %>% filter(sub_grp == "Deprivation" & 
...
chart_9 <- ggplot(trend_data %>% filter(label == "Population" 
                                        & hb == "Scotland"), aes(x = forcats::fct_reorder(quarter_short,
                                               as.numeric(quarter))
                                               , y = crd_rate)) +
  geom_line(aes(colour = label, group=label)) +
  trend_palette +
...
url_list <- data.frame(a = c(1, 2, 3, 4), 
             url = c(paste0("http://www.isdscotland.org/Health-Topics/Quality-Indicators/Publications/",pub_date(end_date, "current"),"/Table1.xls"), 
                     paste0("http://www.isdscotland.org/Health-Topics/Quality-Indicators/Publications/",pub_date(end_date, "current"),"/Table2.xls"),
                     paste0("http://www.isdscotland.org/Health-Topics/Quality-Indicators/Publications/",pub_date(end_date, "current"),"/Table3.xls"),
...
<Div custom-style = "Heading 4">Chart 1: HSMR for deaths within 30-days of admission (Funnel Plot); `r yr(end_date)`</div>
...
<Div custom-style = "Heading 4">Chart 2: Overall crude mortality rates (%) for deaths within 30-days of admission; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
![Trend chart showing crude mortality by quarter for latest five years.](Chart_2.png)
...
<Div custom-style = "Heading 4">Chart 3: Crude mortality rates (%) for deaths within 30-days of admission by type of admission; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Heading 4">Chart 4: Crude mortality rates (%) for deaths within 30-days of admission by specialty group; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Heading 4">Chart 5: Crude mortality rates (%) for deaths within 30-days of admission by age group; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Heading 4">Chart 6: Crude mortality rates (%) for deaths within 30-days of admission by gender; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Heading 4">Chart 7: Crude mortality rates (%) for deaths within 30-days of admission by deprivation; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
Chart 8: Crude mortality rates (%) for deaths within 30-days of ultimate hospital discharge (includes all in-hospital mortality);
Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(21), "short")`p
The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`)
...
Chart 9 shows the trend in overall population mortality for Scotland between `r qtr(start_date_trends, "long")` and `r qtr(start_date_trends + months(21), "long")`. The series, although exhibiting seasonal characteristics, has remained relatively stable over the last three years.

Chart 9: Underlying Population Death Rates (crude rates per 1,000 population);
Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(21), "short")`p
The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`)
![Test Alt Text](Chart_9.png)
...
The latest NHSScotland data from **`r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(21), "short")`** and comparable information by NHS Board of treatment and Hospital are given in the Excel files that accompany this publication.  Please note the following.
...
<Div custom-style = "Heading 3_pgbrk">A3 - Data Quality and Timeliness</div>
...
Death registrations from NRS are linked to hospital admissions in Scotland on a monthly basis. This has enabled the HSMR time series to be extended to include admissions up to `r format(submission_deadline(end_date), "%B %Y") ` in this release.  
...
The previous report, published on `r format(pub_date(end_date, "previous"), "%B %Y")`, presented provisional data for April – June 2018 along with data completeness estimates for those hospitals undergoing PMS implementation. The data for that quarter has now been refreshed to reflect additional returns that have subsequently been submitted for that quarter. The impact of those further submissions suggests that previous completeness estimation was appropriate, and at Scotland level the data were approximately 99% complete. The overall Scottish HSMR for April – June 2018 has reduced to 0.80 from 0.81 (rounded to 2 decimal places).  

Table 3 shows the impact of the refresh on the individual hospital level HSMRs. Completeness levels in the latest period for those NHS Boards undergoing PMS implementation is generally now consistent with levels observed for other NHS Boards.  

The updated analysis suggests that completeness for the majority of all remaining hospitals during April – June 2018 is now in excess of 99%.
