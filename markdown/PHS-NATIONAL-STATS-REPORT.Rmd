...
chart_10 <- ggplot(trend_data %>% filter(label == "Population" 
  ...
  shape_palette +
  trend_palette +
  labs(x = element_blank(), y = "Crude rate per 1,000 people") +
  theme(panel.background = element_blank(),
    panel.grid.minor.x = element_line(size = .15, color = "#C0C0C0"), 
    panel.grid.major.y = element_blank(),
    axis.line.x = element_line(size = .1, color = "#787878"),
...
* Unadjusted hospital mortality remained relatively stable at between 2.6% and 3.3% over the majority of the five-year period from `r format(start_date_trends, "%B %Y")` to `r format(end_date, "%B %Y")`. The relatively small variation shows clear seasonal patterns with increases in mortality during the winter months. Larger increases have been seen during the COVID-19 epidemic in Scotland, however, with unadjusted hospital mortality reaching 5.3% during the first wave and 4.3% during the second. 
...
*	Over three quarters of deaths within 30-days of admission take place in hospital with the other quarter of deaths in community.
...
HSMR quarterly releases from August 2019 onwards are not comparable to earlier releases due to changes in the methodology. These changes were designed to refine the model, and were made in response to a [review](https://www.isdscotland.org/Health-Topics/Quality-Indicators/HSMR/Research-and-Development/) following the end of the Scottish Patient Safety Programme’s aim of reducing hospital mortality by December 2018. 

The main changes made in August 2019 were:

*	Moving to a dynamic three-year base period, advanced by three months with each quarterly publication, ensuring the Scottish HSMR is always representative of current outcomes and reflective of changing case-mix and provision of services.
*	Moving to a twelve-month reporting period – rather than three months – when drawing comparisons against the national average, smoothing out seasonal variation and reducing variation for smaller hospitals.
...
HSMR values are calculated for a one-year period, and can be interpreted as follows: 

* An HSMR value of exactly 1 means that the number of patients who died within 30 days of admission is equal to the number of predicted deaths. In other words, the risk of death matches the Scottish average risk for the categories of patient seen. The HSMR value for Scotland is therefore always 1.00.
* If an HSMR value for a hospital is less than 1: the number of deaths within 30 days of admission for this hospital is fewer than predicted.
* If an HSMR value for a hospital is greater than 1: the number of deaths within 30 days of admission for this hospital is more than predicted.
...
Chart 2 shows the overall crude hospital mortality percentage at Scotland level for deaths within 30 days of admission. The value is generally around 3%, with seasonal increases over the winter months. However, when Scotland went into emergency measures due to COVID-19, hospitals reduced the number of patients admitted and prioritised more seriously ill patients. The crude mortality rate therefore increased substantially in April to June 2020, the first full quarter of emergency measures. Although it has subsequently decreased, the mortality rate remains elevated due to this reduction in patient numbers.
...
This section examines some of the factors that affect the chance of mortality within 30 days of hospital admission. Chart 3 shows the mortality trend for the two types of admission: elective and non-elective. It shows that non-elective patients consistently account for the largest proportion of deaths.
...
Chart 10 shows the trend in overall population mortality for Scotland between `r qtr(start_date_trends, "long")` and `r qtr(start_date_trends + months(57), "long")`. Values have remained relatively stable over the last five years, with some seasonal variation, before increasing in April to June 2020 due to the impact of the COVID-19 pandemic.

<Div custom-style = "Chart Title">Chart 10: Population death rates (crude rates per 1,000 people);
Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`)
...
<Div custom-style = "Heading 2 no pgbrk">Open data</div>
