...
The methodology has been updated to ensure the diagnosis ICD-10 codes for
COVID-19, U07.1 and U07.2 are included in the model. For more
...
this healthcare emergency. As a result, there will be changes in the volumes
...
trends HSMRs should be considered in this context.
...
These statistics are updated every three months and reflect the HSMR for the latest 12 month reporting period when drawing comparisons against the Scottish average, whilst crude mortality data is presented by quarter to show trends. A link to the relevant Excel data table has also been provided alongside each chart.

<Div custom-style = "Heading 2 no pgbrk">How is HSMR calculated?</div>
...
* Over the five year period from `r format(start_date_trends, "%B %Y")` to `r format(end_date, "%B %Y")`, although exhibiting clear seasonal patterns, unadjusted hospital mortality has remained relatively stable at around 3% until April to June 2020, Reflecting the impact of the first wave of the COVID-19 epidemic in Scotland.
...
<Div custom-style = "Heading 1">Comparing HSMR with previous years</div>
During 2018/19, PHS (formerly Information Services Division) undertook a review of the methodology used to calculate HSMR. A [review paper](https://www.isdscotland.org/Health-Topics/Quality-Indicators/HSMR/Research-and-Development/) was subsequently produced with recommendations for refining the model following the end of the Scottish Patient Safety Programme aim. The methodology used by PHS was updated in August 2019. Please note HSMR quarterly releases from August 2019 onwards are not comparable to previous releases.
...
<Div custom-style = "Heading 1">Interpreting HSMR results</div>
...
<Div custom-style = "Heading 1">How HSMR information is used</div>
...
<Div custom-style = "Heading 1">1 Results and Commentary</div>
<Div custom-style = "Heading 2 no pgbrk">Point in Time Comparisons</div>
...
<Div custom-style = "Chart Title">Chart 1: HSMR for deaths within 30-days of admission (Funnel Plot); `r yr(end_date)`</div>
...
<Div custom-style = "Heading 1">2 Trends at Scotland Level</div>

<Div custom-style = "Heading 2 no pgbrk">Scotland Crude Mortality Trends</div>
Chart 2 shows that overall crude hospital mortality (%) at Scotland level from `r qtr(start_date_trends, "long")` to `r qtr(start_date_trends + months(57), "long")`, according to the existing definition of deaths within 30-days of admission, is fluctuating around 3% with clear seasonal patterns exhibited until April to June 2020, reflecting the first full quarter of Scotland going into emergency measures due to COVID-19. This means crude mortality is consistently higher around winter quarters (i.e. October - December and January - March). The crude mortality for July to September 2020 is 3.4%, and although lower than April to June 2020, this is still being driven by a reduction in patient numbers.
<Div custom-style = "Chart Title">Chart 2: Overall crude mortality rates (%) for deaths within 30-days of admission; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
...
<Div custom-style = "Heading 1">3 Scotland (sub-groups)</div>
<Div custom-style = "Heading 2 no pgbrk">Scotland – by type of admission</div>
...
<Div custom-style = "Chart Title">Chart 3: Crude mortality rates (%) for deaths within 30-days of admission by type of admission; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
It shows that patients under the Community specialty have the highest rate of crude mortality within 30-days of admission, followed by medical. Those recorded under Dental, Paediatrics, Women & Newborn specialty having the lowest rate.  
<Div custom-style = "Chart Title">Chart 4: Crude mortality rates (%) for deaths within 30-days of admission by specialty group; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Chart Title">Chart 5: Crude mortality rates (%) for deaths within 30-days of admission by age group; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Chart Title">Chart 6: Crude mortality rates (%) for deaths within 30-days of admission by sex; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Chart Title">Chart 7: Crude mortality rates (%) for deaths within 30-days of admission by deprivation; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
<Div custom-style = "Chart Title">Chart 8: Proportion of deaths within 30-days of admission by place of death; Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`)
...
For the latest quarter (`r qtr(start_date_trends + months(57), "long")`) there were `r format(latest_deaths_admission$deaths, big.mark = ",")` deaths within 30-days of admission to hospital in Scotland, and `r format(latest_deaths_discharge$deaths, big.mark = ",")` deaths within 30-days of ultimate hospital discharge (including all in-hospital deaths). The mortality rates are inevitably higher than the admission-based method, reflecting the longer opportunity for follow-up. The trend is however consistent with the admission based mortality patterns, and show that although exhibiting seasonal variation, hospital mortality had been showing a general decrease over time, until April to June 2020 reflecting the first full quarter of Scotland going into emergency measures due to COVID-19.

<Div custom-style = "Chart Title">Chart 9: Crude mortality rates (%) for deaths within 30-days of ultimate hospital discharge (includes all in-hospital mortality);
...
Chart 10 shows the trend in overall population mortality for Scotland between `r qtr(start_date_trends, "long")` and `r qtr(start_date_trends + months(57), "long")`. The series, although exhibiting seasonal characteristics, has remained relatively stable over the last three years. Although there was an increase in April to June 2020, reflecting the first full quarter of Scotland going into emergency measures due to COVID-19.

<Div custom-style = "Chart Title">Chart 10: Underlying population death rates (crude rates per 1,000 people);
Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")`p</div>
<Div custom-style = "centre_justify">The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`)
...
These results have also been published in an interactive dashboard format. A link to the latest dashboard is available on the [HSMR webpage](https://beta.isdscotland.org/find-publications-and-data/health-services/hospital-care/hospital-standardised-mortality-ratios/).


<Div custom-style = "Heading 1">Refinements to HSMR methodology</div>
Since the HSMR statistics were first released in 2009, PHS (formerly Information Services Division) have periodically reviewed the model methodology to ensure that it continues to be robust and that comparisons which are made against the national average continue to be appropriate and relevant for each point in time. 
...
**Lucinda Lawrie, Senior Information Analyst**  
Quality Indicators  
Phone:  0131 275 7929  

**Catherine Perkins, Senior Information Analyst**  
Quality Indicators  
Phone:  0131 275 7710 
...
<Div custom-style = "Heading 2 no pgbrk">Further Information</div>
Further Information can be found on the [PHS website](https://beta.isdscotland.org/).  
...
<Div custom-style = "Heading 2 no pgbrk">Rate this publication</div>
...
<Div custom-style = "Heading 2 no pgbrk">Appendix 1 – Background information</div>
...
<Div custom-style = "Chart Title">Table 3: Key comparisons between the favoured methods for hospital mortality indicators in Scotland and England.</div>
...
The first Official Statistics release of the information was in June 2010, when a set of abbreviated summary tables were published on a dedicated website and linked to the main PHS site. The timing of the publication has been altered to better synchronise with the availability of death data from National Records of Scotland and to optimise the timeliness of reporting. See the section on Timeliness for further information. 
...
A new patient management system (TrakCare PMS) has been implemented in twelve NHS Boards, with the exception of NHS Dumfries & Galloway and NHS Western Isles.
