...
output: 
  word_document:
    reference_docx: "phs-accredited-stats-report_hsmr_template.dotx"
    
---

```{r LoadConfig, echo=FALSE, message=FALSE, warning=FALSE,error=FALSE, include=FALSE}
...
shape_palette <- scale_shape_manual(values = c(16, 15, 17, 3, 4, 8, 18,1))
# ----
## FUNNEL PLOT ##
smr_data <- smr_data %>%
  mutate(
    flag_above_control = smr > ucl,
    flag_below_control = smr < lcl,
    flag_above_warning = smr > uwl & smr <= ucl,
    flag_below_warning = smr < lwl & smr >= lcl
  )

# Step 1: Create specific flags for colouring points
smr_data <- smr_data %>%
  mutate(colour_flag = case_when(
    flag_above_control == TRUE | flag_below_control == TRUE ~ "Control Limit",
    flag_above_warning == TRUE | flag_below_warning == TRUE ~ "Warning Limit",
    TRUE ~ "Scotland"
  ))

# Step 2: Build the funnel plot
funnel_plot <- ggplot(smr_data %>% filter(location_type == "hospital"), aes()) +
  geom_line(aes(x = pred, y = ucl, colour = "Control Limit")) +
  geom_line(aes(x = pred, y = uwl, colour = "Warning Limit")) +
  geom_line(aes(x = pred, y = lwl, colour = "Warning Limit")) +
  geom_line(aes(x = pred, y = lcl, colour = "Control Limit")) +
  geom_line(aes(x = pred, y = 1,   colour = "Scotland")) +
  
  # Coloured points based on flag
  geom_point(aes(x = pred, y = smr, colour = colour_flag), show.legend = FALSE) +
  
  # Labels: below
  geom_label_repel(
    data = smr_data %>% filter(location_type == "hospital" & flag_below == TRUE),
  ...
  # Labels: above
  geom_label_repel(
    data = smr_data %>% filter(location_type == "hospital" & flag_above == TRUE),
    aes(x = pred, y = smr, label = location_name),
    nudge_y = 2,
    size = 4,
    box.padding = 0.5,
  ...
  scale_color_manual(
    values = c(
      "Scotland" = "#006ba4",
      "Control Limit" = "#c85302",
      "Warning Limit" = "#ffbc79"
    ),
    labels = c(
      "Scotland" = "Scotland",
      "Control Limit" = "Upper/Lower Control Limit",
      "Warning Limit" = "Upper/Lower Warning Limit"
    ),
...
```{r table_eng_comp, echo = FALSE, message = FALSE, warning = FALSE, error =  FALSE}
...
",
        "CCS group (primary diagnosis)/ Specialty (medical or surgical) / age / sex / admitted from / number and severity of prior morbidities in the previous 5-years / number and severity of co-morbidities on admitting episode / number of emergency admissions in the previous 12 months / inpatient or day case / type of admission (elective / non-elective)/ deprivation")
...
<Div custom-style = "Heading 1 alt">Introduction</div>
...
Quarterly HSMR publications have been released since 2009, with periodic reviews of the methodology. 
In August 2019, there was significant change to methodology and any data **published from August 2019 onwards cannot be compared to prior releases using a different methodology. ** The HSMR methodology had been updated to ensure the diagnosis ICD-10 codes for COVID-19, U07.1 to U07.7, are included in the model. For more information please refer to the [HSMR COVID-19 Methodology Update paper ](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-supporting-resources/).
...
The most recent review took place in August 2025 and has been reflected in the methodology since the November 2025 publication, ensuring that the performance of the HSMR model continues to be robust. These changes are relatively minor and mainly reflect the best practice in statistical modelling and clinical relevance. Therefore, no changes are expected to the crude mortality rates or to the volume of activity which is used to calculate the HSMRs. For more information, please refer to [2025 HSMR Model Review](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-2025/) and [2025 Technical Document](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-2025/).

This release provides HSMRs for the period `r yr(end_date)`, using validated hospital SMR01 returns as at `r format(submission_deadline(end_date), "%d %B %Y")`. This information has previously been released to NHS Boards as management information. Additional information and commentary on patterns of mortality by key demographic factors is included for the period `r format(start_date_trends, "%B %Y")` to `r format(end_date, "%B %Y")` so that changes over time can be evaluated. There is also extensive information on the development of the measure and how it compares to similar information used in other parts of the UK.

<Div custom-style = "Heading 1 alt">Hospital Standardised Mortality Ratios</div>
...
Safety is a priority ambition for the **[Healthcare Quality Strategy for NHSScotland (external website)](https://www.gov.scot/publications/healthcare-quality-strategy-nhsscotland/)**. A lower hospital mortality should reflect work in individual hospitals to review mortality, and reflect reductions in serious adverse events and infections under the Scottish Patient Safety Programme and other improvement initiatives.
...
<Div custom-style = "Heading 1 alt">Main points</div>

`r funnel_text(smr_data, "above")[1]`
`r funnel_text(smr_data, "below")[1]`
* Unadjusted hospital mortality remained relatively stable at between 2.6% and 3.3% over the majority of the five-year period from `r format(start_date_trends, "%B %Y")` to `r format(end_date, "%B %Y")`. The relatively small variation shows clear seasonal patterns with increases in mortality during the winter months. It is important to note that large increases occurred during the COVID-19 pandemic in Scotland, with crude mortality rates reaching 5.4% during the first wave, 4.4% during the second wave. 
...
*	Around 4 in 5 deaths within 30-days of admission take place in hospital while the remaining deaths take place in the community.
...
<Div custom-style = "Heading 1 alt">1 HSMR results and commentary</div>
...
Chart 1 shows the funnel plot for the latest 12-month period. The control limits for this chart are three standard deviations above or below the Scottish average and shown in red. `r funnel_text(smr_data, "both")[3]`

<Div custom-style = "Table or chart caption">Chart 1: HSMR for deaths within 30-days of admission, `r yr(end_date)` p</div>
<Div custom-style = "centre_justify"> ![This chart shows each hospitals HSMR for the latest 12 month period plotted on a funnel plot.](Chart_1.png) The data relating to this chart can be found in [Table1](`r url_list[1,"url"]`). **p** Provisional </div>

<Div custom-style = "Heading 1 alt">2 Crude mortality trends</div>
...
<Div custom-style = "Heading 2 less spacing">Scotland – overall</div>
...
<Div custom-style = "Table or chart caption less spacing">Chart 2: Overall crude mortality rates (%) for deaths within 30-days of admission, Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")` p</div>
<Div custom-style = "centre_justify">![Trend chart showing crude mortality by quarter for latest five years.](Chart_2.png)The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). Table 2 also provides crude mortality charts for each NHS Board and hospital. **p** Provisional</div>



<Div custom-style = "Heading 2 page break">Scotland – by type of admission</div>
...
![Trend chart showing crude mortality by type of admission for the last five years](Chart_3.png)The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 2 page break">Scotland – by specialty</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing crude mortality by specialty group for the last five years](Chart_4.png) The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 2 page break">Scotland – by age</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing crude mortality by age group for the last five years](Chart_5.png) The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 2 page break">Scotland – by sex</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing crude mortality by sex for the last five years](Chart_6.png) The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 2 page break">Scotland – by Scottish Index of Multiple Deprivation</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing crude mortality by deprivation for the last five years](Chart_7.png) The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 2 page break">Scotland – by place of death</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing proportion of deaths by place of admission for the last five years](Chart_8.png) The data relating to this chart can be found in [Table 2](`r url_list[2, "url"]`). **p** Provisional</div>


<Div custom-style = "Heading 1 alt">3 Alternative crude mortality measures</div>
<Div custom-style = "Heading 2 less spacing">In-patient mortality and deaths within 30-days of discharge</div>
...
<Div custom-style = "Table or chart caption less spacing">Chart 9: Crude mortality rates (%) for deaths within 30-days of ultimate hospital discharge (includes all in-hospital mortality), Scotland, `r qtr(start_date_trends, "short")` to `r qtr(start_date_trends + months(57), "short")` p</div>
<Div custom-style = "centre_justify">
![Trend chart showing crude mortality for deaths within 30 days of discharge for the last five years](Chart_9.png) The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`). Table 3 also provides charts for mortality within 30 days of discharge for each NHS Board. **p** Provisional</div>
...
<Div custom-style = "centre_justify">
![Trend chart showing crude population mortality for the last five years](Chart_10.png) The data relating to this chart can be found in [Table 3](`r url_list[3, "url"]`). Table 3 also provides population-based mortality charts for each NHS Board area. **p** Provisional</div>




<Div custom-style = "Heading 1 alt">Hospital and NHS Board level</div>
...
*	**NHS Greater Glasgow and Clyde**: in order to reflect current service configuration, the HSMRs for Stobhill Hospital and Glasgow Royal Infirmary have been combined as have the activity for Royal Alexandra Hospital and Vale of Leven. In addition, Queen Elizabeth University Hospital activity includes that of the New Victoria and West Glasgow hospitals. These changes have been applied to all time points retrospectively back to the initial reporting period. 

Individual hospital level data for any combined sites are available on request.

<Div custom-style = "Heading 1 alt">Refinements to HSMR methodology</div>
<Div custom-style = "Heading 2 less spacing">2019 Model Review</div>
...
<Div custom-style = "Heading 2 less spacing">2025 Model Review</div>
As a part of this publication, we are using the updated 2025 HSMR Model. The main points of the HSMR model changes and considerations are as follows:

**Changes**

*	**Comorbidity weighting index changed from Charlson to the Elixhauser Comorbidity Index:** This index marginally improves the model and will align with best practice.

*	**Prior morbidities within 1 year removed as a predictor from the model:** The two prior morbidities predictors - within 1 year and within 5 years - prior to the last continuous inpatient stay (CIS), provide similar information in the model. Therefore, to ensure no statistical overfitting in the model, the factor prior morbidities within 1 year will be removed.Prior morbidity within 5 years will continue to be included due to its clinical usefulness for providing context for patient's longer term medical history.

* **Primary Diagnosis Groupings** have been updated from 141 groups to 143, in line with NHS England's SHMI indicator recommendations.

* **Refactoring the model to more representative and clinically significant groups - Sex and primary diagnosis groups (PDGs):** In the model, although there is no statistical difference from rebasing the variables in the model, for best practice, we have decided to use more representative and clinically significant groups than the default currently in use. 

**Considerations**

* **Sex retained as a predictor in the model:** Although sex was found to have no statistically significant impact on model performance, it will remain in the model to support clinical interpretation and relevance.

* **Ethnicity data is not appropriate to include in the model:** PHS are periodically reviewing the quality and completeness of the data and aim to trial its inclusion as a part of a crude mortality analysis by ethnicity in a future model review. 

* **Continue to use the first episode of a patient's last Continuous Inpatient Stay (CIS):** We considered to use the last episode instead of first, however decided to continue to use first episode as this ensures that we continue to capture the whole patient journey and initial diagnosis.

More details of the review work and recommendations are available in the [2025 HSMR Model Review](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-2025/). A [Technical Document](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-2025/) is also available that describes the current HSMR methodology in more detail. [Interpretation Guidance and Frequently Asked Questions](https://publichealthscotland.scot/publications/hospital-standardised-mortality-ratios-hsmr-quick-access-guides/) documents answer questions on the differences between the previous and current methodologies.

<Div custom-style = "Heading 1 alt">Glossary</div>
...
<Div custom-style = "Heading 1 alt">Contact</div>


**Cemre Su Lode, Senior Information Analyst**
Quality Indicators

**Paul Ogharanduku, Senior Information Analyst**  
Quality Indicators 

[phs.QualityIndicators@phs.scot](mailto:phs.QualityIndicators@phs.scot)

<Div custom-style = "Heading 1 nopagebreak">Further information</div>

Further Information can be found on the [PHS website](https://publichealthscotland.scot/).  
For more information on HSMR see the [HSMR section of our website](https://publichealthscotland.scot/our-areas-of-work/acute-and-emergency-services/hospital-standardised-mortality-ratios-hsmr/hsmr-overview/). The code used to produce this publication can be accessed in this [GitHub repository (external website)](https://github.com/Public-Health-Scotland/hsmr).
...
<Div custom-style = "Heading 1 nopagebreak">Data files</div>
...
<Div custom-style = "Heading 1 nopagebreak">Open data</div>
...
<Div custom-style = "Heading 1 nopagebreak">Rate this publication</div>
...
<Div custom-style = "Heading 1 alt">Appendices</div>
...
The table below shows a summary of some of the key comparisons between the English and Scottish approaches. 

<Div custom-style = "Table or chart caption">Key comparisons between the favoured methods for hospital mortality indicators in Scotland and England.</div>

```{r kable_table_eng_comp, echo=FALSE}

knitr::kable(table_eng_comp, col.names = c("**Key Comparison Parameters**","**Scotland (HSMR)**",
"**England and Wales Summary Hospital-level Mortality Indicator (SHMI)**",
"**England and Wales Dr Foster (HSMR)**"))
```

<Div custom-style = "Heading 2 page break">Appendix 2 – Quarterly process </div>
...
There are two key stages to this quarterly process which include other indicators of quality. 
...
<Div custom-style = "Heading 2 page break">Appendix 3 – Data quality and timeliness</div>
...
In August 2016, PHS published the findings of their most recent [quality assurance assessment (external website)](https://webarchive.nrscotland.gov.uk/20231129143210mp_/https://www.isdscotland.org/Products-and-Services/Data-Quality/docs/Assessment-of-SMR01-Data-2014-15-report-181019.pdf)  which was undertaken during 2014-15 to ensure that SMR01 (General / Acute Inpatient and Day Case) data items are being recorded consistently and to a high standard throughout NHS Scotland. The report shows that:  
...
More information about SMR completeness is available on [our website](https://publichealthscotland.scot/services/data-management/data-management-in-secondary-care-hospital-activity/scottish-morbidity-records-smr/completeness/).
...
`r paste0("Release of HSMR at Scotland, NHS Board and Hospital levels for the period ", yr(end_date),". Also includes analyses of crude mortality trends over the longer term from ", qtr(start_date_trends, "long")," to ", qtr(start_date_trends + months(57), "long"),".")`
...
`r format(submission_deadline(end_date), "%d %B %Y")`

**Release date**

`r format(pub_date(end_date = end_date, pub = "current"), "%d %B %Y")`
...
`r paste0("Hospital Standardised Mortality Ratios (HSMR) for the latest 12 month period ", yr(end_date), ". Quarterly crude mortality trends from ", qtr(start_date_trends, "long")," to ", qtr(start_date_trends + months(57), "long"), " (one quarter reporting lag).")`
...
`r paste0("Approximately ", completeness(first_day=qtr_start, "current", "scotland"), " for the latest quarter.")`
...
Assessment report published (ref no. 249). Confirmed as Accredited Official statistics April 2013.

**Last published**

`r format(pub_date(end_date = end_date, pub = "previous"), "%d %B %Y")`

**Next published**

`r format(pub_date(end_date = end_date, pub = "next"), "%d %B %Y") `
...
<Div custom-style = "Heading 2 page break">Appendix 5 – Early access details</div>
...
<Div custom-style = "Heading 2 page break">Appendix 6 – PHS and Official Statistics</div>
